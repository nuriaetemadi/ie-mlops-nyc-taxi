...
def load_data(limit: int = 100_000) -> pd.DataFrame:
    ...
    print(f"[SUCCESS] Loaded {len(df):,} rows after filtering")
    return df
...
def train_and_log(X_train_dicts, y_train, X_val_dicts, y_val):
    ...
    with mlflow.start_run() as run:
        ...
        print(f"[SUCCESS] RMSE: {rmse:.2f}  MAE: {mae:.2f}  R2: {r2:.3f}")
        ...
        with open("run_id.txt", "w") as f:
            f.write(run_id)
        print(f"[INFO] Saved run_id.txt (run: {run_id})")
        print("[INFO] View MLflow UI: http://localhost:5000")
        print("[INFO] Logged single Pipeline artifact 'model'")
        return run_id
